...
class VA_Server:
    ...
    def __init__(self, job_config):
        ...
        self.scheduler = FlowMatchScheduler(shift=self.job_config.snr_shift,
                                            sigma_min=0.0,
                                            extra_one_step=True)
        action_snr_override = os.environ.get("ACTION_SNR_SHIFT")
        if action_snr_override:
            old_shift = self.job_config.action_snr_shift
            self.job_config.action_snr_shift = float(action_snr_override)
            print(f"[ACTION_SNR_SHIFT] override action_snr_shift {old_shift} -> {self.job_config.action_snr_shift}", flush=True)
        ...
        max_memory = None
        max_memory_list = getattr(job_config, 'max_memory_list', None)
        if max_memory_list:
            import torch as _torch
            n_gpus = _torch.cuda.device_count()
            max_memory = {i: max_memory_list[i] if i < len(max_memory_list) else max_memory_list[-1] for i in range(n_gpus)}
        ...
        self.transformer = load_transformer(
            ...
            device_map=getattr(job_config, 'device_map', None),
            max_memory=max_memory,
            attn_mode="torch"
        )
        self._pre_shard_lora_wrapped = 0
        pre_shard_lora_rank = int(getattr(job_config, 'pre_shard_lora_rank', 0) or 0)
        if pre_shard_lora_rank > 0:
            try:
                from wan_va.lora import inject_lora
            except Exception:
                from lora import inject_lora
            self._pre_shard_lora_wrapped = inject_lora(
                self.transformer,
                rank=pre_shard_lora_rank,
                alpha=int(getattr(job_config, 'pre_shard_lora_alpha', pre_shard_lora_rank)),
                dropout=float(getattr(job_config, 'pre_shard_lora_dropout', 0.0)),
                include_ffn=bool(getattr(job_config, 'pre_shard_lora_include_ffn', False)),
            )
            if getattr(job_config, 'rank', 0) == 0:
                print(f"[pre-shard-lora] wrapped {self._pre_shard_lora_wrapped} linears", flush=True)
        if bool(getattr(job_config, 'pre_shard_apply_ac', False)):
            from distributed.fsdp import apply_ac
            apply_ac(self.transformer)
            if getattr(job_config, 'rank', 0) == 0:
                print("[pre-shard-ac] activation checkpointing enabled", flush=True)
    ...
    def _infer(self, obs, frame_st_id=0, wm_config=None, wm_context=None):
        ...
        # Watermark injection: mix keyed reference into action noise
        if wm_config is not None and wm_context is not None:
            from wan_va.wm.watermark import mix_internal_noise
            actions = mix_internal_noise(
                actions,
                sample_rate_hz=wm_config.control_freq,
                config=wm_config,
                context=wm_context,
                active_channel_ids=self.job_config.used_action_channel_ids,
                frame_chunk_size=frame_chunk_size,
                action_per_frame=self.action_per_frame,
            )
            self._last_wm_noise = actions.detach().clone()
        ...
        with (
            ...
            for i, t in enumerate(tqdm(timesteps)):
                ...
                video_noise_pred = self.transformer(
                    ...
                    action_mode=False)
                video_noise_pred = video_noise_pred.to(latents.device)
            ...
            for i, t in enumerate(tqdm(action_timesteps)):
                ...
                action_noise_pred = self.transformer(
                    ...
                    cache_name=self.cache_name,
                    action_mode=True)
                action_noise_pred = action_noise_pred.to(actions.device)
    ...
    def sample_actions_from_noise(self, z_init, frame_st_id=0, num_steps=None, shift_override=None):
        """Differentiable action denoising from given initial noise.

        Runs the action FM loop WITHOUT modifying KV-cache (update_cache=0).
        Requires video denoising to have already been run and cached.

        Args:
            z_init: [B, C, F, H, 1] initial action noise (requires_grad ok)
            frame_st_id: frame start index for positional encoding
            num_steps: override number of denoising steps (default: config value)

        Returns:
            actions: [B, C, F, H, 1] decoded action tensor (differentiable w.r.t. z_init)
        """
        from torch.utils.checkpoint import checkpoint as ckpt_fn

        frame_chunk_size = self.job_config.frame_chunk_size
        action_inference_step = num_steps if num_steps is not None else self.job_config.action_num_inference_steps

        # Precompute timestep schedule as plain tensors (no scheduler state dependency)
        self.action_scheduler.set_timesteps(action_inference_step, shift=shift_override)
        sigmas = self.action_scheduler.sigmas.detach().clone()  # [n_steps]
        # Append sigma=0 as terminal (matches _infer's pad + scheduler.step to_final logic)
        sigmas = torch.cat([sigmas, sigmas.new_zeros(1)])
        timesteps_sched = self.action_scheduler.timesteps.detach().clone()  # sigmas * 1000
        ...
        def _denoise_step(actions_in, sigma_curr, sigma_next, t_curr):
            """Single denoising step with direct Euler integration."""
            noisy_latents = actions_in
            if action_cond is not None:
                noisy_latents = torch.cat([action_cond.to(noisy_latents.dtype), noisy_latents[:, :, 1:]], dim=2)
            mask = self.action_mask.to(noisy_latents.device)[None, :, None, None, None]
            noisy_latents = noisy_latents * mask.to(noisy_latents.dtype)

            timesteps = torch.ones([noisy_latents.shape[2]],
                                   dtype=torch.float32,
                                   device=noisy_latents.device) * t_curr
            if action_cond is not None:
                timesteps = torch.cat([torch.zeros(1, device=timesteps.device), timesteps[1:]])

            grid_id = get_mesh_id(
                noisy_latents.shape[-3], noisy_latents.shape[-2],
                noisy_latents.shape[-1], 1, 1, frame_st_id, action=True
            ).to(noisy_latents.device)

            if self.use_cfg:
                action_input = {
                    'noisy_latents': noisy_latents.repeat(2, 1, 1, 1, 1),
                    'text_emb': torch.cat([self.prompt_embeds.to(self.dtype).detach(),
                                           self.negative_prompt_embeds.to(self.dtype).detach()], dim=0),
                ...
                action_input = {
                    ...
                    'text_emb': self.prompt_embeds.to(self.dtype).detach(),
                }

            # Cast to model dtype for transformer (differentiable in PyTorch)
            model_dtype = self.dtype
            action_input = {k: v.to(model_dtype) if v.is_floating_point() else v
            ...
            # Move back to input device (handles multi-GPU split) and cast dtype
            action_noise_pred = action_noise_pred.to(device=actions_in.device, dtype=actions_in.dtype)
            ...
            # Direct Euler step: x_next = x + (sigma_next - sigma_curr) * v
            dt = sigma_next - sigma_curr
            out = actions_in + dt * action_noise_pred
            if action_cond is not None:
                out = torch.cat([action_cond.to(out.dtype), out[:, :, 1:]], dim=2)
            return out

        force_action_ckpt = os.environ.get("WAN_ACTION_CKPT", "0").lower() in ("1", "true", "yes", "on")
        use_ckpt = force_action_ckpt or (action_inference_step > 10)
        for i in range(len(sigmas) - 1):
            s_curr = sigmas[i]
            s_next = sigmas[i + 1]
            t_curr = timesteps_sched[i]
            if use_ckpt:
                actions = ckpt_fn(_denoise_step, actions, s_curr, s_next, t_curr, use_reentrant=False)
            else:
                actions = _denoise_step(actions, s_curr, s_next, t_curr)

        # Apply action mask (non-in-place for gradient flow)
        mask = self.action_mask.to(actions.device)[None, :, None, None, None]
        actions = actions * mask.to(actions.dtype)
        return actions

    def infer_with_watermark(self, obs, frame_st_id, wm_config, wm_context):
        """Run inference with watermark injection, returning both raw and processed actions.

        Returns:
            dict with:
                action: postprocessed numpy actions for env execution
                raw_actions: [B, C, F, H, 1] tensor before postprocess (for MAP target)
                wm_noise: [B, C, F, H, 1] the watermarked initial noise (MAP seed)
        """
        action_np, _ = self._infer(obs, frame_st_id=frame_st_id,
                                    wm_config=wm_config, wm_context=wm_context)
        return {
            'action': action_np,
            'raw_actions': getattr(self, '_last_raw_actions', None),
            'wm_noise': getattr(self, '_last_wm_noise', None),
        }
