...
from lora import inject_lora, mark_only_lora_trainable, merge_lora_state_dict
from utils import (
    init_logger, 
    logger, 
    get_mesh_id, 
...
class Trainer:
    def __init__(self, config):
        ...
        # Optional LoRA: inject adapters into attention (and optionally FFN) linears of every
        # block BEFORE activation checkpointing / FSDP sharding. Base weights are frozen; only
        # LoRA params train. At save time the adapters are merged back into plain Linear weights
        # so the checkpoint loads with the stock WanTransformer3DModel (see save_checkpoint).
        ...
        if self.use_lora:
            n = inject_lora(
                ...
                include_ffn=getattr(config, "lora_include_ffn", False),
            )
            logger.info(f"LoRA enabled: wrapped {n} linear layers "
                        f"(rank={getattr(config, 'lora_rank', 16)}, alpha={getattr(config, 'lora_alpha', 16)})")
        ...
        if getattr(self, "use_lora", False):
            trainable, total = mark_only_lora_trainable(self.transformer)
    ...
    def save_checkpoint(self,):
        ...
        try:
            ...
            # Fold LoRA adapters back into plain Linear weights so the saved checkpoint loads
            # with the stock WanTransformer3DModel (eval/server path is unchanged).
            if getattr(self, "use_lora", False):
