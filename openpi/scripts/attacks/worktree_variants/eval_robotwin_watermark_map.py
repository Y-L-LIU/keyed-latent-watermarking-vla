...
def _recover_noise_map_jax(
    ...
    *,
    ...
    restart_latents = []
    restart_energies = []
    for _ in range(int(args.map_num_starts)):
        z0 = rng.standard_normal(size=(1, horizon, raw_dim)).astype(np.float32)
    ...
    return np.asarray(restart_latents[best], dtype=np.float32), energies, best
