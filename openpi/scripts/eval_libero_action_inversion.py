...
class InversionChunkTrace:
    ...
    # Phase-1 recovery study: optional per-chunk noise recovered by each of the
    # 4 inversion modes (full+ODE, partial+ODE, full+MAP, partial+MAP). Filled
    # only when --save-all-inversion-modes is set; zeros otherwise.
    recovered_noise_full_ode: np.ndarray = dataclasses.field(
        default_factory=lambda: np.zeros((0, 0), dtype=np.float32)
    )
    recovered_noise_partial_ode: np.ndarray = dataclasses.field(
        default_factory=lambda: np.zeros((0, 0), dtype=np.float32)
    )
    recovered_noise_full_map: np.ndarray = dataclasses.field(
        default_factory=lambda: np.zeros((0, 0), dtype=np.float32)
    )
    recovered_noise_partial_map: np.ndarray = dataclasses.field(
        default_factory=lambda: np.zeros((0, 0), dtype=np.float32)
    )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--watermark-dims", type=str, default=None,
                        help="comma-separated latent dims to key, e.g. '0,1,2,3,4,5,6' (default: all dims)")
    parser.add_argument(
    ...
    # Observation-tied keying (distillation-survivable mode). With keying_mode=observation the
    # keyed reference r(k*,o) is a deterministic function of the (quantized) per-chunk observation
    # state, regenerated identically at injection and verification from chunk_observation_state.
    parser.add_argument("--keying-mode", choices=("nonce", "observation"), default="nonce")
    parser.add_argument("--obs-key", default="observation/state")
    parser.add_argument("--obs-proj-dims", default="0,1,2", help="comma-separated state dims to hash; empty=all")
    parser.add_argument("--obs-quantization", type=float, default=0.08)
    ...
    # Phase-1 recovery study: compute ALL 4 inversion modes per chunk
    # (full+ODE / partial+ODE / full+MAP / partial+MAP) instead of just the one
    # selected by the mutually-exclusive flags above. Stores each as a separate
    # field `chunk_recovered_noise_<mode>` so the 8-cell table can be filled
    # from one rollout. Best paired with --resume-from-rollouts to avoid re-rolling.
    parser.add_argument("--save-all-inversion-modes", action="store_true")
...
def _obs_seed_for_config(
    reference_config: online_eval.wm.InternalNoiseWatermarkConfig,
    observation_state: np.ndarray | None,
) -> int | None:
    """Compute the observation-tied seed for a chunk, or None for nonce keying.

    Used at both injection and verification so the keyed reference r(k*, o) is
    regenerated from the *same* (quantized) observation the policy was keyed on.
    """
    if getattr(reference_config, "keying_mode", "nonce") != "observation":
        return None
    if observation_state is None:
        raise ValueError("observation keying requires a per-chunk observation_state")
    return online_eval.wm.compute_obs_seed(
        np.asarray(observation_state),
        quantization=float(reference_config.obs_quantization),
        proj_dims=reference_config.obs_proj_dims,
    )
...
def _retarget_chunk_references(
    ...
    *,
    ...
    for trace in chunk_traces:
        if trace.selected and trace.executed_steps > 0:
            obs_seed = _obs_seed_for_config(reference_config, getattr(trace, "observation_state", None))
            context = online_eval.wm.WatermarkContext(
                chunk_index=trace.chunk_index, episode_nonce=episode_nonce, obs_seed=obs_seed
            )
    ...
    try:
        ...
        while t < max_steps + args.num_steps_wait:
            ...
            if not action_plan:
                ...
                inj_obs_seed = _obs_seed_for_config(reference_config, element.get("observation/state"))
                context = online_eval.wm.WatermarkContext(
                    chunk_index=chunk_index, episode_nonce=episode_nonce, obs_seed=inj_obs_seed
                )
                ...
                # Phase-1 recovery study: compute the 3 other inversion modes per
                # selected chunk and stash for active_trace. Mutually-exclusive
                # flags are toggled briefly so each helper passes its arg check.
                extra_modes = {
                    "full_ode": np.zeros_like(recovered_noise, dtype=np.float32),
                    "partial_ode": np.zeros_like(recovered_noise, dtype=np.float32),
                    "full_map": np.zeros_like(recovered_noise, dtype=np.float32),
                    "partial_map": np.zeros_like(recovered_noise, dtype=np.float32),
                }
                if getattr(args, "save_all_inversion_modes", False) and selected:
                    _saved_flags = (args.fm_channel_inverse, args.fm_full_latent_map,
                                    args.fm_latent_map, args.fm_latent_posterior)
                    # 1) full+ODE: reverse-Euler on full raw_action
                    args.fm_channel_inverse = False
                    args.fm_full_latent_map = False
                    args.fm_latent_map = False
                    args.fm_latent_posterior = False
                    extra_modes["full_ode"] = np.asarray(_recover_noise_from_actions(
                        detector_policy, obs=element, raw_actions=raw_action_chunk, args=args),
                        dtype=np.float32)
                    # 2) partial+ODE: zero-pad NORMALIZED env-visible 7D to 32D -> reverse-Euler
                    # (channel-complete heuristic diverges numerically; zero-pad is stable.
                    #  env action is post-normalize → must apply same normalization the
                    #  detector saw during training, otherwise inversion is in wrong space.)
                    _y_obs_norm = _normalize_channel_observation(detector_policy, observed_action_chunk)
                    a_full = np.zeros_like(raw_action_chunk, dtype=np.float32)
                    a_full[..., : _y_obs_norm.shape[-1]] = np.asarray(_y_obs_norm, dtype=np.float32)
                    extra_modes["partial_ode"] = np.asarray(_recover_noise_from_actions(
                        detector_policy, obs=element, raw_actions=a_full, args=args),
                        dtype=np.float32)
                    # 3) full+MAP: MAP on full action
                    args.fm_full_latent_map = True
                    payload_fm = _recover_noise_from_full_action_latent(
                        detector_policy, obs=element, raw_action_chunk=raw_action_chunk, args=args)
                    extra_modes["full_map"] = np.asarray(
                        payload_fm["recovered_noise"], dtype=np.float32)
                    args.fm_full_latent_map = False
                    # 4) partial+MAP: MAP on env-visible 7D
                    args.fm_latent_map = True
                    payload_pm = _recover_noise_from_channel_observation_latent(
                        detector_policy, obs=element,
                        env_action_chunk=observed_action_chunk,
                        raw_action_chunk=raw_action_chunk, args=args)
                    extra_modes["partial_map"] = np.asarray(
                        payload_pm["recovered_noise"], dtype=np.float32)
                    # restore
                    (args.fm_channel_inverse, args.fm_full_latent_map,
                     args.fm_latent_map, args.fm_latent_posterior) = _saved_flags
                ...
                active_trace = {
                    ...
                    "recovered_noise_full_ode": extra_modes["full_ode"],
                    "recovered_noise_partial_ode": extra_modes["partial_ode"],
                    "recovered_noise_full_map": extra_modes["full_map"],
                    "recovered_noise_partial_map": extra_modes["partial_map"],
            ...
            if active_segment["executed_steps"] == active_segment["planned_steps"] or done:
                ...
                inversion_traces.append(
                    InversionChunkTrace(
                        ...
                        recovered_noise_full_ode=np.asarray(
                            active_trace.get("recovered_noise_full_ode", np.zeros_like(active_trace["recovered_noise"])),
                            dtype=np.float32),
                        recovered_noise_partial_ode=np.asarray(
                            active_trace.get("recovered_noise_partial_ode", np.zeros_like(active_trace["recovered_noise"])),
                            dtype=np.float32),
                        recovered_noise_full_map=np.asarray(
                            active_trace.get("recovered_noise_full_map", np.zeros_like(active_trace["recovered_noise"])),
                            dtype=np.float32),
                        recovered_noise_partial_map=np.asarray(
                            active_trace.get("recovered_noise_partial_map", np.zeros_like(active_trace["recovered_noise"])),
                            dtype=np.float32),
                    )
                )
                active_segment = None
        ...
        payload.update(
            {
                ...
                "chunk_recovered_noise_full_ode": np.asarray(
                    [getattr(t, "recovered_noise_full_ode", np.zeros_like(t.recovered_noise))
                     for t in inversion_traces], dtype=np.float32,
                ),
                "chunk_recovered_noise_partial_ode": np.asarray(
                    [getattr(t, "recovered_noise_partial_ode", np.zeros_like(t.recovered_noise))
                     for t in inversion_traces], dtype=np.float32,
                ),
                "chunk_recovered_noise_full_map": np.asarray(
                    [getattr(t, "recovered_noise_full_map", np.zeros_like(t.recovered_noise))
                     for t in inversion_traces], dtype=np.float32,
                ),
                "chunk_recovered_noise_partial_map": np.asarray(
                    [getattr(t, "recovered_noise_partial_map", np.zeros_like(t.recovered_noise))
                     for t in inversion_traces], dtype=np.float32,
                ),
    ...
    for task_id in range(task_start, task_stop):
        ...
        try:
            ...
            for episode_idx in range(num_trials):
                ...
                # Observation-tied keying: the true reference r(k*,o) is a function of the
                # per-chunk observation, not of the injected trace. Regenerate it from each
                # rollout's recorded observation_state so a PLAIN (no-injection) distilled
                # student is scored against the correct key. No-op for nonce keying.
                if getattr(reference_config, "keying_mode", "nonce") == "observation":
                    plain_inversion = _retarget_chunk_references(
                        plain_inversion, reference_config=reference_config,
                        sample_rate_hz=args.sample_rate_hz, episode_nonce=episode_nonce,
                    )
                    marked_inversion = _retarget_chunk_references(
                        marked_inversion, reference_config=reference_config,
                        sample_rate_hz=args.sample_rate_hz, episode_nonce=episode_nonce,
                    )
