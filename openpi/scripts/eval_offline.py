...
def _validate_args(args: argparse.Namespace) -> None:
    ...
    if int(args.latent_map_iters) <= 0:
        raise ValueError("latent_map_iters must be > 0.")
    if float(args.latent_map_lr) <= 0.0:
        raise ValueError("latent_map_lr must be > 0.")
    if int(args.map_num_starts) <= 0:
...
def _map_num_starts(args: argparse.Namespace) -> int:
    return int(getattr(args, "map_num_starts", 1))


def _map_random_seed(args: argparse.Namespace) -> int:
    return int(getattr(args, "map_random_seed", 0))


def _latent_map_iters(args: argparse.Namespace) -> int:
    return int(getattr(args, "latent_map_iters", 100))


def _latent_map_lr(args: argparse.Namespace) -> float:
    return float(getattr(args, "latent_map_lr", 1e-1))


def _map_restart_seed(
    trace: _base.InversionChunkTrace,
    *,
    episode_nonce: int,
    args: argparse.Namespace,
) -> int:
    prompt_hash = sum(ord(ch) for ch in str(trace.prompt)) & 0xFFFFFFFF
    seed = (
        np.uint32(_map_random_seed(args))
        + np.uint32(int(episode_nonce) * 1009)
        + np.uint32(int(trace.chunk_index) * 9176)
        + np.uint32(prompt_hash)
    )
    return int(seed)
...
def _posterior_chain_init_mode_from_map_restart(
    trace: _base.InversionChunkTrace,
    *,
    best_restart_index: int,
) -> str:
    if int(best_restart_index) == 0:
        return _saved_trace_chain_init_mode(trace)
    return "map"
...
def _clip_jax_grad_by_global_norm(grad, *, max_norm: float):
    if float(max_norm) <= 0.0:
        return grad
    flat = grad.reshape((grad.shape[0], -1))
    norms = _base.jnp.linalg.norm(flat, axis=1, keepdims=True)
    eps = _base.jnp.asarray(_base.jnp.finfo(grad.dtype).eps, dtype=grad.dtype)
    scales = _base.jnp.minimum(1.0, float(max_norm) / _base.jnp.maximum(norms, eps))
    return grad * scales.reshape((grad.shape[0],) + (1,) * (grad.ndim - 1))
...
def _pytorch_energy(
    ...
    *,
    ...
    pred_obs = a_pred[:, :, : y_obs.shape[-1]] if mode == "channel" else a_pred
    ...
    tether_loss = (
        0.5 * tether_weight * torch.mean(torch.square(z - z_anchor))
        if tether_weight > 0.0 and z_anchor is not None
...
def _run_pytorch_posterior_from_saved_map(
    ...
    *,
    ...
    y_obs = torch.as_tensor(y_obs, dtype=torch.float32, device=device)
    time_grid = torch.as_tensor(time_grid, dtype=torch.float32, device=device)
    ...
    for step_idx in range(total_steps):
        ...
        grad_z = torch.nan_to_num(torch.autograd.grad(energy, z)[0])
        grad_z = _clip_pytorch_grad_by_global_norm(grad_z, max_norm=_posterior_grad_clip_norm(args))
        noise = torch.randn_like(z) if step_size > 0.0 else torch.zeros_like(z)
        z = (z - 0.5 * step_size * grad_z + (step_size**0.5) * noise).detach()
        z = torch.nan_to_num(z)
        if step_idx >= int(args.posterior_burnin) and (step_idx - int(args.posterior_burnin)) % int(args.posterior_thinning) == 0:
    ...
    energies = torch.stack(sample_energies, dim=0).reshape(-1)
    return np.asarray(z_samples[0].cpu(), dtype=np.float32), np.asarray(energies.cpu(), dtype=np.float32)


def _map_observation_operator(*, mode: str, obs_dim: int, args: argparse.Namespace):
    if mode == "channel":
        return _base.ChannelObservation(obs_sigma=float(args.obs_sigma))
    return _base.ChannelObservation(channel_idx=tuple(range(int(obs_dim))), obs_sigma=float(args.obs_sigma))


def _run_pytorch_map_restarts(
    policy,
    *,
    model_inputs,
    y_obs: torch.Tensor,
    ...
    device = policy._pytorch_device
    obs_op = _map_observation_operator(mode=mode, obs_dim=int(y_obs.shape[-1]), args=args)
    solver = _base.FMLatentMAPSolver(
        policy._model,
        obs_op,
        _base.FMLatentMAPConfig(
            num_iters=_latent_map_iters(args),
            lr=_latent_map_lr(args),
    ...
    y_obs = torch.as_tensor(y_obs, dtype=torch.float32, device=device)
    time_grid = torch.as_tensor(time_grid, dtype=torch.float32, device=device)
    z_starts = _build_map_restart_initial_latents(
        z_seed,
        num_starts=_map_num_starts(args),
        seed=int(rng_seed),
    )
    restart_maps: list[np.ndarray] = []
    restart_energies: list[float] = []
    for z_start in z_starts:
        z_init = torch.from_numpy(np.asarray(z_start, dtype=np.float32)).to(device=device, dtype=torch.float32)[None, ...]
        ...
        restart_maps.append(np.asarray(z_map[0].cpu(), dtype=np.float32))
        restart_energies.append(float(energy.item()))
    energies = np.asarray(restart_energies, dtype=np.float32)
    best_index = int(np.argmin(energies)) if energies.size else -1
    return np.asarray(restart_maps, dtype=np.float32), energies, best_index
...
def _jax_energy(
    ...
    *,
    ...
    tether_weight = _posterior_map_tether_weight(args)
    tether_loss = (
        0.5 * tether_weight * _base.jnp.mean(_base.jnp.square(z - z_anchor))
        if tether_weight > 0.0 and z_anchor is not None
        else _base.jnp.asarray(0.0, dtype=z.dtype)
    )
    return obs_loss + prior_loss + tether_loss
...
def _run_jax_posterior_from_saved_map(
    ...
    *,
    ...
    def ula_step(carry, _step_idx):
        ...
        return (_base.jnp.nan_to_num(z_next), key), _base.jnp.nan_to_num(z_next)
    ...
    return np.asarray(z_samples[0], dtype=np.float32), np.asarray(sample_energies, dtype=np.float32)


def _run_jax_map_restarts(
    policy,
    *,
    model_inputs,
    y_obs,
    ...
    for z_start in z_starts:
        z_map = _base._optimize_latent_with_adam_jax(
            init_noise=_base.jnp.asarray(np.asarray(z_start, dtype=np.float32)[None, ...], dtype=_base.jnp.float32),
            loss_fn=energy_fn,
            num_steps=_latent_map_iters(args),
            learning_rate=_latent_map_lr(args),
        )
        z_map = _base.jnp.nan_to_num(z_map)
        restart_maps.append(np.asarray(z_map[0], dtype=np.float32))
...
def _augment_trace_with_saved_map_posterior(
    ...
    *,
    ...
    z_seed = np.asarray(trace.recovered_noise, dtype=np.float32)
    map_rng_seed = _map_restart_seed(trace, episode_nonce=episode_nonce, args=args)
    ...
    if policy._is_pytorch_model:
        ...
        map_restart_recovered_noise, map_restart_energies, map_best_restart_index = _run_pytorch_map_restarts(
            policy,
            model_inputs=model_inputs,
            y_obs=y_obs,
            time_grid=time_grid,
            ...
            mode=mode,
            args=args,
            rng_seed=map_rng_seed,
        )
        z_init = np.asarray(map_restart_recovered_noise[map_best_restart_index], dtype=np.float32)
        ...
        map_restart_recovered_noise, map_restart_energies, map_best_restart_index = _run_jax_map_restarts(
            policy,
            model_inputs=model_inputs,
            y_obs=y_obs,
            time_grid=time_grid,
    ...
    map_restart_recovered_noise = np.asarray(map_restart_recovered_noise, dtype=np.float32)
    map_restart_energies = np.asarray(map_restart_energies, dtype=np.float32)
    z_map_best = np.asarray(map_restart_recovered_noise[map_best_restart_index], dtype=np.float32)
    ...
    chain_init_mode = _posterior_chain_init_mode_from_map_restart(trace, best_restart_index=map_best_restart_index)
    return dataclasses.replace(
        trace,
        recovered_noise=z_map_best,
        map_restart_recovered_noise=map_restart_recovered_noise,
        map_restart_energies=map_restart_energies,
        map_best_restart_index=int(map_best_restart_index),
...
def _payload_with_augmented_posterior(
    ...
    updated["chunk_recovered_noise"] = np.asarray(
        [trace.recovered_noise for trace in traces],
        dtype=np.float32,
    )
    updated["chunk_map_restart_recovered_noise"] = np.asarray(
        [trace.map_restart_recovered_noise for trace in traces],
        dtype=np.float32,
    )
    updated["chunk_map_restart_energies"] = np.asarray(
        [trace.map_restart_energies for trace in traces],
        dtype=np.float32,
    )
    updated["chunk_map_best_restart_index"] = np.asarray(
        [trace.map_best_restart_index for trace in traces],
        dtype=np.int32,
    )
...
def _augment_rollout_file(
    ...
    *,
    ...
    updated_payload["offline_saved_map_posterior_source"] = np.asarray("best_chunk_map_restart")
    updated_payload["offline_saved_map_refined_map"] = np.asarray(True)
    updated_payload["latent_map_iters"] = np.asarray(int(args.latent_map_iters), dtype=np.int32)
    updated_payload["latent_map_lr"] = np.asarray(float(args.latent_map_lr), dtype=np.float32)
    updated_payload["map_num_starts"] = np.asarray(int(args.map_num_starts), dtype=np.int32)
    updated_payload["map_random_seed"] = np.asarray(int(args.map_random_seed), dtype=np.int32)
    ...
    updated_payload["posterior_map_tether_weight"] = np.asarray(float(args.posterior_map_tether_weight), dtype=np.float32)
    updated_payload["posterior_grad_clip_norm"] = np.asarray(float(args.posterior_grad_clip_norm), dtype=np.float32)
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        for path in rollout_paths:
            out_path = _output_rollout_path(path, input_root=args.rollout_dir, output_root=args.output_dir)
            summaries.append(
    ...
    print(f"map_num_starts={int(args.map_num_starts)}")
